"""SalesAgent: composes and sends cold emails, then polls the inbox and
...
Email copy is fully deterministic: it is assembled from the active
Campaign's templates (see pipeline/campaigns.py), not generated by a model.
An LLM drafting path (Hugging Face Mistral-7B) existed here until it was
removed as unreachable -- nothing had called it since the screenshot/
checklist email redesign, so every email already came from the templates.
