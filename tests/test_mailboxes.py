...
def mailbox_env(tmp_path, monkeypatch):
    ...
    # The send path validates the preview URL over HTTP before emailing
    # (tests/test_sales_agent_preview_safety.py covers that); these tests are
    # about mailbox selection, so hand every lead a preview that passes.
    monkeypatch.setattr(sales_agent, "_validate_preview_link_for_send", lambda link: link or "https://preview.test")
    db.init_db()
    return tmp_path
