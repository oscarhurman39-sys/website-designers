...
# Load .env.test before any pipeline module is imported anywhere in the
# test session: pipeline/config.py reads os.environ into module-level
# constants at import time, so this has to happen at conftest collection
# time, not inside a fixture. override=True so .env.test always wins over
# a real .env or already-set shell env vars during the test session.
